...
In this experiment, as the number of edges increases, the approximation ratios
for approximation 2 and approximation 3 decrease toward 1. Approximation one
stays close to 1 the whole time but trends slightly up.

The data suggests that denser graphs have larger MVCs, and larger graphs have
more possible MVCs which is why the random picking approximations expected
performance improve with increased edges. This also shows that the greedy
...
In this experiment, as the number of nodes increases while edges stay at 5, the
expected approximation ratios for approximation 2 and approximation 3 increase.
Approximation 1 stays near 1 again.

The result suggest that making more nodes makes the graph sparser. Sparse graphs
are where random choice approximations can get worse in performance because they
can waste picks more easily. This also shows that approximation 1's performance
...
The results suggest that random choice algorithms perform the better as the
number of edges increase. This also shows that greedy approximation algorithms
are consistent when finding a minimum vertex cover.
